...
then
        ...
        #删除旧项目文件目录：yunmas_man
        rm -fr yunmas_man
        #备份webapps下的旧war包：yunmas_man.war
        mv yunmas_man.war $path/dbbak/yunmas_man.war.$(date +%Y%m%d%H%M%S%N)
        #复制新的war包：yunmas_man.war 到webapps路径下
        cp $path/appBase/yunmas_man.war yunmas_man.war
        #休眠1s
        sleep 1
        cd ../bin
