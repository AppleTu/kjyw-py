...
then
        ...
        #删除旧项目文件目录：yunmas_biz
        rm -fr yunmas_biz
        #备份webapps下的旧war包：yunmas_biz.war
        mv yunmas_biz.war $path/dbbak/yunmas_biz.war.$(date +%Y%m%d%H%M%S%N)
        #复制新的war包：yunmas_biz.war 到webapps路径下
        cp $path/appBase/yunmas_biz.war yunmas_biz.war
        #休眠1s
        sleep 1
        cd ../bin
