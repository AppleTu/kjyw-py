...
then
        ...
        #删除旧项目文件目录：yunmas_sms
        rm -fr yunmas_sms
        #备份webapps下的旧war包：yunmas_sms.war
        mv yunmas_sms.war $path/dbbak/yunmas_sms.war.$(date +%Y%m%d%H%M%S%N)
        #复制新的war包：yunmas_sms.war 到webapps路径下
        cp $path/appBase/yunmas_sms.war yunmas_sms.war
        #休眠1s
        sleep 1
        cd ../bin
