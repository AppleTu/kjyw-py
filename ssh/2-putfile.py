...
for index, row in hostList.iterrows():
    ...
    localfile = str(row["from"])
    remotefile = str(row["to"])
    remotebakfile = str(row["bakdir"])

    s = Server.Server(ip, 22, username, password)  # 实例化
    exists_localfile = s.exists_local(localfile)  # 检查本地路径文件是否存在
    if exists_localfile == True:  # 如果存在，继续
        try:
            print(s.connect())  # 创建一个连接
            print(s.open_ssh())  # 开启ssh
            print(s.open_sftp())  # 开启sftp
            exists_remotefile = s.exists_remote(remotefile)  # 检查远程路径文件是否存在
            print(s.mv_remote(remotefile, remotebakfile))  # 原文件备份至dbbak
